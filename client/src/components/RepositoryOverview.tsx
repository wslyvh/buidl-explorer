...
const GET_REPOS = gql`
	query($first: Int!, $type: String, $startCursor: String, $endCursor: String) {
		searchRepositories(first: $first, type: $type, startCursor: $startCursor, endCursor: $endCursor) {
			repositoryCount
			pageInfo {
				hasNextPage
				hasPreviousPage
...
class RepositoryOverview extends Component<IProps, IState> {
	...
	public render() {
		...
		const issueType = this.state && this.state.type ? this.state.type : "good-first-issues";
				...
				<div style={{ textAlign: "center" }}>
					...
					<div>
						{languages.map((language: any, index: number) => {
							return (
								<Tag key={index} color={language.color} style={{ padding: "10px 15px 30px 15px", fontSize: 12 }}>
						...
						<a href="#" hidden={this.state && this.state.filter === ""} onClick={() => this.filter("")}>
							Clear
						</a>
					</div>
					<br />
					...
					<SearchRepositoryQuery query={GET_REPOS} variables={{ first: limit, type: issueType }}>
								...
								fetchMore({
									...
									updateQuery: (previousResult, { fetchMoreResult }) => {
										if (fetchMoreResult) {
											...
											previousResult.searchRepositories.nodes = [...previousResult.searchRepositories.nodes, ...fetchMoreResult.searchRepositories.nodes];
										}

										return previousResult;
									},
									variables: { first: limit, type: issueType, startCursor: pageInfo.endCursor }
								});
							}
